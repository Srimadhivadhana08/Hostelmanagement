...
const statusLabel: Record<string, string> = {
  PENDING: "Pending HOD",
  PENDING_WARDEN: "Needs Warden Review",
  APPROVED: "Approved",
  REJECTED: "Rejected",
  EXPIRED: "Expired",
}
...
export default function WardenLeaveApprovalsPage() {
  ...
  const [filterStatus, setFilterStatus] = useState("PENDING_WARDEN")
  ...
  const loadCounts = async () => {
    try {
      const results = await Promise.all(
        ["PENDING_WARDEN", "APPROVED", "REJECTED"].map(async s => {
    ...
    try {
      ...
      if (res.ok) {
        toast.success(newStatus === "APPROVED"
          ? "✅ Leave approved — student has been notified"
          : "❌ Leave rejected"
        )
        setSelected(null)
        setRemarks("")
  ...
  const pendingCount = counts["PENDING_WARDEN"] || 0
  const tabs = [
    { value: "PENDING_WARDEN", label: "Needs Warden Review" },
  ...
  return (
    <div className="p-6 lg:p-8">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Warden Leave Approvals</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            Final approval for HOD-approved outpass requests
          </p>
        </div>
        {pendingCount > 0 && (
          <div className="flex items-center gap-2 rounded-xl border border-blue-200 bg-blue-50 px-4 py-2">
            <Clock className="h-4 w-4 text-blue-600" />
            <span className="text-sm font-bold text-blue-700">{pendingCount} awaiting your review</span>
          </div>
        )}
      </div>

      {/* Workflow Info */}
      <div className="mt-5 rounded-xl border border-emerald-100 bg-emerald-50 p-4 flex items-start gap-3">
        <Info className="h-4 w-4 text-emerald-600 flex-shrink-0 mt-0.5" />
        <div>
          <p className="text-sm font-semibold text-emerald-800">Warden Approval Stage (Stage 2 of 2)</p>
          <p className="text-xs text-emerald-700 mt-0.5">
            You only see requests that have already been approved by the HOD. Your decision is final — <strong>Approve</strong> to grant leave or <strong>Reject</strong> to deny.
          </p>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-md rounded-2xl bg-card border border-border p-6 shadow-xl max-h-[85vh] overflow-y-auto">
            ...
            <div className="space-y-3 text-sm">
              ...
              {/* HOD Remarks (from previous stage) */}
              {selected.hodRemarks && (
                <div className="rounded-lg border border-blue-200 bg-blue-50 p-3">
                  <p className="text-xs font-semibold text-blue-700 mb-1 flex items-center gap-1">
                    <CheckCircle2 className="h-3 w-3" /> HOD Remarks
                    {selected.hodApprovedAt && (
                      <span className="font-normal ml-1">
                        · {new Date(selected.hodApprovedAt).toLocaleDateString("en-IN", { day: "numeric", month: "short" })}
                      </span>
                    )}
                  </p>
              ...
              {/* Warden Remarks Input */}
              {selected.status === "PENDING_WARDEN" && (
                <div>
                  <label className="block text-xs font-medium text-muted-foreground mb-1">
                    Your Remarks <span className="text-muted-foreground/60">(optional — visible to student)</span>
                  </label>
                  <textarea
                    value={remarks}
            ...
            {selected.status === "PENDING_WARDEN" ? (
              <div className="flex gap-3 mt-4">
                ...
                <button
                  onClick={() => handle(selected.id, "APPROVED")}
                  disabled={updating}
                  className="flex-1 flex items-center justify-center gap-2 rounded-lg bg-emerald-50 border border-emerald-200 py-2.5 text-sm font-semibold text-emerald-600 hover:bg-emerald-100 transition-colors"
                >
      ...
      <div className="mt-5 space-y-3">
        {loading ? (
          ...
          <div className="flex flex-col items-center py-16 rounded-xl border border-dashed border-border bg-card">
            {filterStatus === "PENDING_WARDEN" ? (
              <>
                <CheckCircle2 className="h-10 w-10 text-emerald-400 mb-3" />
                <p className="text-foreground font-semibold">Queue is clear!</p>
                <p className="text-sm text-muted-foreground mt-1">No requests waiting for your approval.</p>
              </>
            ) : (
              <>
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="font-semibold text-foreground">
                    {o.user.name}
                    {o.user.room && (
                      <span className="text-xs font-normal text-muted-foreground ml-2">· Rm {o.user.room.roomNumber}</span>
                    )}
                  </p>
                  ...
                  {o.hodRemarks && (
                    <p className="text-xs text-blue-600 mt-0.5 flex items-center gap-1">
                      <Info className="h-3 w-3" /> HOD: "{o.hodRemarks.slice(0, 50)}{o.hodRemarks.length > 50 ? "…" : ""}"
                    </p>
                  )}
                </div>
              </div>
